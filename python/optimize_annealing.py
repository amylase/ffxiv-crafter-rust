...
def create_objective():
    ...
    def _objective(trial: optuna.Trial):
        ...
        end_temperature = trial.suggest_float("end_temperature", 10 ** -4, 10 ** -4, log=True)
        temperature_scale = trial.suggest_float("start_temperature", 0.01, 0.01, log=True)
        max_quality_scaling = trial.suggest_float("max_quality_scaling", 1.1, 1.1)
        add_proba = trial.suggest_float("add_proba", 0.1, 0.1)
        remove_proba = trial.suggest_float("remove_proba", 0.1, 0.1)
        swap_proba = trial.suggest_float("swap_proba", 0.05, 0.05)
        params = {}
        ...
        with open(metrics_path, encoding="utf-8") as f:
            metrics = json.load(f)
        return metrics["metrics"]["max_quality_rate"]
    return _objective
